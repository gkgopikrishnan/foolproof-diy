# Snowflake Objects Management Utility

## Overview

This utility script is designed to manage and organize Snowflake database objects, ensuring that their DDL (Data Definition Language) statements are properly versioned and prepared for check-in to Git. The script reads an input file listing database objects, retrieves their DDL statements, and saves them in a structured format with versioning. Errors encountered during the process are logged for review.
...
### 2. Script Execution
- **Initialization:**
  - Reads from `object_list.txt` and initializes paths for various files:
...
### 6. Summary
- **Processing:** After processing all objects, the script writes output files in their respective directories and logs any errors in `object_missed.txt`.
- **Purpose:** The generated SQL files and error logs aid in reviewing, documenting, or migrating database objects.

## Detailed Flow

1. **Read Input:**
   - Open `object_list.txt`.
   - Extract and de-duplicate object names.
...
3. **Write Output:**
   - Save formatted DDLs in directories organized by object type.
   - Include versioning in filenames.

4. **Handle Errors:**
   - Log any issues in `object_missed.txt`.
